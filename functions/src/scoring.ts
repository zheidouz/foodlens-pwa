...
export interface HealthScoreResult {
  ...
  ecoscore: string | null;    // "A"–"E" or null
  nova_group: number | null;  // 1–4 or null
  good_factors: Factor[];
  bad_factors: Factor[];
}
...
function getNutrientValue(nutriments: Record<string, number>, key: string): number {
  // Try typical key variants from Open Food Facts
  const variants = [key, key.replace(/-/g, "_"), `_${key}`];
  for (const v of variants) {
    if (nutriments[v] !== undefined && nutriments[v] !== null) return nutriments[v];
  }
  return 0;
}

function scoreSugar(nutriments: Record<string, number>): number {
  const sugar = getNutrientValue(nutriments, "sugars_100g");
  for (const t of SUGAR_THRESHOLDS) {
    if (sugar <= t.max) return t.penalty;
  }
  return -15;
}

function scoreSatFat(nutriments: Record<string, number>): number {
  const fat = getNutrientValue(nutriments, "saturated-fat_100g");
  for (const t of SAT_FAT_THRESHOLDS) {
    if (fat <= t.max) return t.penalty;
  }
  return -10;
}

function scoreSodium(nutriments: Record<string, number>): number {
  const sodium = getNutrientValue(nutriments, "sodium_100g");
  for (const t of SODIUM_THRESHOLDS) {
    if (sodium <= t.max) return t.penalty;
  }
  return -10;
}

function scoreFiber(nutriments: Record<string, number>): number {
  const fiber = getNutrientValue(nutriments, "fiber_100g");
  for (const t of FIBER_THRESHOLDS) {
    if (fiber >= t.min) return t.bonus;
  }
  return 0;
}

function scoreProtein(nutriments: Record<string, number>): number {
  const protein = getNutrientValue(nutriments, "proteins_100g");
  for (const t of PROTEIN_THRESHOLDS) {
    if (protein >= t.min) return t.bonus;
  }
  return 0;
}

function scoreNova(novaGroup: number | null | undefined): number {
  if (novaGroup === null || novaGroup === undefined) return 0;
  switch (novaGroup) {
...
function scoreAdditives(additives: string[] | undefined): number {
...
function calculateNutriscore(nutriments: Record<string, number>): string {
  ...
  if (satFat > 0) nPoints += satFat <= 1 ? 1 : satFat <= 2 ? 2 : satFat <= 3 ? 3 : satFat <= 4 ? 4 : satFat <= 5 ? 5 : satFat <= 6 ? 6 : satFat <= 7 ? 7 : satFat <= 8 ? 8 : satFat <= 9 ? 9 : 10;
  if (sodium > 0) nPoints += sodium <= 0.09 ? 1 : sodium <= 0.18 ? 2 : sodium <= 0.27 ? 3 : sodium <= 0.36 ? 4 : sodium <= 0.45 ? 5 : sodium <= 0.54 ? 6 : sodium <= 0.63 ? 7 : sodium <= 0.72 ? 8 : sodium <= 0.81 ? 9 : 10;

  // Positive points (P)
  let pPoints = 0;
  if (fiber > 0) pPoints += fiber <= 0.9 ? 1 : fiber <= 1.9 ? 2 : fiber <= 2.8 ? 3 : fiber <= 3.9 ? 4 : 5;
  if (protein > 0) pPoints += protein <= 1.6 ? 1 : protein <= 3.2 ? 2 : protein <= 4.8 ? 3 : protein <= 6.4 ? 4 : 5;
...
export function calculateHealthScore(product: ProductInput): HealthScoreResult {
